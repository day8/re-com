...
(def table-args-desc
  [{:name :model                     :required true                     :type "vector of maps | atom"    :validate-fn vector-or-atom?              :description "one element for each row in the table."}
   ...
   ;; Verify number of parameters on 3 following:
   {:name :on-click-row              :required false                    :type "function"                 :validate-fn ifn?                         :description "This function is called when the user clicks a row. Called with the row map and the mouse event. Do not use for adjusting row styles, use styling instead."}
   {:name :on-enter-row              :required false                    :type "function"                 :validate-fn ifn?                         :description "This function is called when the user's mouse pointer enters a row. Called with the row map and the mouse event. Do not use for adjusting row styles, use styling instead."}
   {:name :on-leave-row              :required false                    :type "function"                 :validate-fn ifn?                         :description "This function is called when the user's mouse pointer leaves a row. Called with the row map and the mouse event. Do not use for adjusting row styles, use styling instead."}
