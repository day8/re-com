(ns re-com.validate
  (:require
    ...
    [re-com.util           :refer [deref-or-value component-path]] ;; TODO: Remove `component-path` when replacing with Reagent version
...
(defn arg-names-valid?
  ...
  (or (superset? defined-args passed-args)
      (let [missing-args (remove defined-args passed-args)]
        (log-error "Invalid argument(s): " missing-args " at " (component-path (reagent/current-component))))))
...
(defn required-args-passed?
  ...
  (or (superset? passed-args required-args)
      (let [missing-args (remove passed-args required-args)]
        (log-error "Missing required argument(s): " missing-args " at " (component-path (reagent/current-component))))))
...
(defn validate-fns-pass?
  ...
  (let [validate-arg (fn [[_ v-arg-def]]
                       (let [arg-name        (:name v-arg-def)
                             ...
                             comp-path       (str " at " (component-path (reagent/current-component)))]
                         ...
                         (cond
                           ...
                           (map?   validate-result)  ((if (= (:status validate-result) :warning) log-warning log-error)
                                                       log-msg-base
