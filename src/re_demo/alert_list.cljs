...
(defn alert-list-demo
  ...
  (let [alerts       (reagent/atom [])]
    (add-alert alerts 0 :danger-modern  {:heading "Woa! something bad happened" :body "Next time you should take more care pressing that button! Did you read the fine print?  No, I didn't think so."})
    (add-alert alerts 1 :info-modern    {:heading "No Wait!" :body "The rain in Spain often falls on the mountatins too."})
    (add-alert alerts 2 :info-modern    {:heading "Here's some info" :body "The rain in Spain falls mainly on the plain."})
    (add-alert alerts 3 :warning-modern {:heading "\"Oh bother\", said Pooh. And then ..." :body "\"Some people care too much. I think it's called love.\""})
                               ...
                               :children [[title2 "Demo"]
                                          [p "To insert alerts at the top of the list, click "
                                           [button
                                            :label "Add alert"
                                            :style {:width "100px"}
                                            :on-click #(add-alert alerts (gensym) :info-modern {:heading "New alert" :body "This alert was added by the \"Add alert\" button."})]]
